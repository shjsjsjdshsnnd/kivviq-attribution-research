...
function validateTimingValue(
  ...
  if (!record(value) || !nonEmpty(value.state)) {
    add(
      ...
      path,
      "explicit timing state is required",
    );
    return;
  }
  if (value.state === "SPECIFIED") {
    if (!("value" in value)) {
      add(
        issues,
        "MISSING_TIMING_VALUE",
        path + ".value",
        "specified state requires a value",
      );
      return;
    }
  ...
  if (value.state === "UNKNOWN") {
    if (!nonEmpty(value.reason)) {
      add(
        issues,
        "INVALID_UNKNOWN_TIMING",
        path + ".reason",
        "unknown state requires a reason",
      );
    }
    return;
  }
  if (value.state === "ABSENT") {
    if (value.reason !== undefined && !nonEmpty(value.reason)) {
      add(
        issues,
        "INVALID_ABSENT_TIMING",
        path + ".reason",
        "reason must be non-empty when supplied",
      );
    }
    return;
  }
  if (value.state === "NOT_APPLICABLE") {
    if (!nonEmpty(value.reason)) {
      add(
        issues,
        "INVALID_NOT_APPLICABLE_TIMING",
        path + ".reason",
        "not-applicable state requires a reason",
      );
    }
    return;
  }
  add(
    issues,
    "UNKNOWN_TIMING_VALUE_STATE",
    path + ".state",
    "unsupported timing state",
  );
}
...
function validateAbsoluteTime(
  ...
  if (!record(value) || !nonEmpty(value.kind)) {
    add(
      ...
      path,
      "absolute time kind is required",
    );
    return;
  }
  if (value.kind === "UTC") {
    if (!validUtc(value.at)) {
      add(
        ...
        "INVALID_UTC_TIME",
        path + ".at",
        "must be valid ISO-8601 UTC ending in Z",
      );
    }
  ...
  if (value.kind === "LOCAL") {
    if (!record(value.at)) {
      add(
        ...
        "INVALID_LOCAL_TIME",
        path + ".at",
        "local business time is required",
      );
      return;
    }
  ...
  add(
    issues,
    "UNKNOWN_ABSOLUTE_TIME_KIND",
    path + ".kind",
    "unsupported absolute time kind",
  );
}
...
function validateOffset(
  ...
  if (value.kind === "ELAPSED") {
    if (!["SECOND", "MINUTE", "HOUR"].includes(String(value.unit))) {
      add(
        issues,
        "INVALID_ELAPSED_UNIT",
        path + ".unit",
        "unsupported elapsed-time unit",
      );
    }
    return;
  }
  if (value.kind === "CALENDAR") {
    if (!["DAY", "WEEK", "MONTH"].includes(String(value.unit))) {
      add(
        issues,
        "INVALID_CALENDAR_UNIT",
        path + ".unit",
        "unsupported calendar unit",
      );
    }
    return;
  }
  add(
    issues,
    "UNKNOWN_TEMPORAL_OFFSET_KIND",
    path + ".kind",
    "unsupported offset kind",
  );
}
...
function validateRequestedStart(
  ...
  if (!record(value) || !nonEmpty(value.kind)) {
    add(
      ...
      path,
      "requested start kind is required",
    );
    return;
  }
  ...
  if (value.kind === "EVENT_RELATIVE") {
    if (!["AFTER_EVENT", "BEFORE_EVENT"].includes(String(value.relation))) {
      add(
        ...
        path + ".relation",
        "unsupported event relation",
      );
    }
    if (!nonEmpty(value.eventId))
      add(
        issues,
        "INVALID_EVENT_ID",
        path + ".eventId",
  ...
  if (value.kind === "TRIGGER_RELATIVE") {
    if (!nonEmpty(value.triggerId))
      add(
        issues,
        "INVALID_TRIGGER_ID",
        path + ".triggerId",
        "trigger identity is required",
      );
    return;
  }
  if (value.kind === "ACTION_RELATIVE") {
    if (
      ...
      add(
        issues,
        "INVALID_ACTION_RELATION",
        path + ".relation",
        "unsupported Action timing relation",
      );
    }
    if (!nonEmpty(value.actionId))
      add(
        issues,
        "INVALID_ACTION_REFERENCE",
        path + ".actionId",
        "Action identity is required",
      );
  ...
  add(
    issues,
    "UNKNOWN_REQUESTED_START_KIND",
    path + ".kind",
    "unsupported requested start kind",
  );
}
...
function validateEffectiveStart(
  ...
  if (!record(value) || !nonEmpty(value.kind)) {
    add(
      ...
      path,
      "effective start kind is required",
    );
    return;
  }
  ...
  add(
    issues,
    "UNKNOWN_EFFECTIVE_START_KIND",
    path + ".kind",
    "unsupported effective start kind",
  );
}
...
function validateDuration(
  ...
  if (value.kind === "ELAPSED" || value.kind === "CALENDAR") {
    ...
    if (
      value.kind === "ELAPSED" &&
      !["SECOND", "MINUTE", "HOUR"].includes(String(value.unit))
    ) {
      add(
        issues,
        "INVALID_ELAPSED_UNIT",
        path + ".unit",
        "unsupported elapsed duration unit",
      );
    }
    if (
      value.kind === "CALENDAR" &&
      !["DAY", "WEEK", "MONTH"].includes(String(value.unit))
    ) {
      add(
        issues,
        "INVALID_CALENDAR_UNIT",
        path + ".unit",
        "unsupported calendar duration unit",
      );
    }
    if (
      !["DECISION_TIME", "REQUESTED_START", "EFFECTIVE_START"].includes(
        String(value.anchor),
      )
    ) {
      add(
        issues,
        "INVALID_DURATION_ANCHOR",
        path + ".anchor",
        "unsupported duration anchor",
      );
    }
    return;
  }
...
function validateEnd(
  ...
  if (value.kind !== "DERIVE_FROM_DURATION") {
    add(
      issues,
      "UNKNOWN_END_RULE_KIND",
      path + ".kind",
      "unsupported end rule kind",
    );
  }
}
...
function validateTermination(
  ...
  if (!record(value) || !nonEmpty(value.kind)) {
    add(
      issues,
      "INVALID_TERMINATION_CONDITION",
      path,
      "termination condition kind is required",
    );
    return;
  }
  if (value.kind === "STATE") {
    if (!record(value.condition) || !nonEmpty(value.condition.kind)) {
      add(
        issues,
        "INVALID_STATE_TERMINATION",
        path + ".condition",
        "typed state condition is required",
      );
      return;
    }
    const condition = value.condition;
    if (condition.kind === "METRIC_THRESHOLD") {
      if (!nonEmpty(condition.metricId))
        add(
          issues,
          "INVALID_METRIC_ID",
          path + ".condition.metricId",
          "metric identity is required",
        );
      if (
        !["LT", "LTE", "EQ", "GTE", "GT"].includes(String(condition.operator))
      ) {
        add(
          issues,
    ...
    if (
      condition.kind === "ACTION_STARTS" ||
      condition.kind === "ACTION_COMPLETES"
    ) {
      if (!nonEmpty(condition.actionId))
        add(
          issues,
          "INVALID_ACTION_REFERENCE",
          path + ".condition.actionId",
          "Action identity is required",
        );
      return;
    }
    add(
      issues,
      "UNKNOWN_STATE_TERMINATION",
      path + ".condition.kind",
      "unsupported state condition",
    );
    return;
  }
  if (value.kind === "COMPOSITE") {
    if (
      !["FIRST_OF", "LAST_OF", "ALL_REQUIRED"].includes(String(value.operator))
    ) {
      add(
        issues,
        "INVALID_TERMINATION_OPERATOR",
        path + ".operator",
        "unsupported composition operator",
      );
    }
    if (!Array.isArray(value.conditions) || value.conditions.length < 2) {
      add(
        issues,
        "INVALID_TERMINATION_COMPOSITION",
        path + ".conditions",
        "composite termination requires at least two conditions",
      );
      return;
    }
  ...
  add(
    issues,
    "UNKNOWN_TERMINATION_KIND",
    path + ".kind",
    "unsupported termination condition kind",
  );
}
...
function validateRecurrenceBoundary(
  ...
  if (!record(value) || !nonEmpty(value.kind)) {
    add(
      issues,
      "INVALID_RECURRENCE_BOUNDARY",
      path,
      "boundary kind is required",
    );
    return;
  }
  if (value.kind === "OPEN_ENDED") {
    if (value.explicitlyOpenEnded !== true) {
      add(
        issues,
        "ACCIDENTAL_OPEN_RECURRENCE",
        path + ".explicitlyOpenEnded",
        "open-ended recurrence must be explicit",
      );
    }
    return;
  }
  if (value.kind === "BOUNDED") {
    if (value.recurrenceStart !== undefined)
    ...
    if (
      ...
      (!Number.isInteger(value.maxOccurrences) || value.maxOccurrences <= 0)
    ) {
      add(
        issues,
    ...
    if (
      ...
      add(
        ...
        path,
        "bounded recurrence requires recurrenceEnd or maxOccurrences",
      );
    }
    return;
  }
  add(
    issues,
    "UNKNOWN_RECURRENCE_BOUNDARY",
    path + ".kind",
    "unsupported recurrence boundary kind",
  );
}
...
function validateRecurrence(
  ...
  if (!nonEmpty(frequency.kind)) {
    add(
      ...
      path + ".frequency.kind",
      "frequency kind is required",
    );
  } else if (frequency.kind === "DAILY") {
    if (!Number.isInteger(frequency.interval) || frequency.interval <= 0)
      add(
        issues,
    ...
    if (
      ...
      add(
        ...
        path + ".frequency.daysOfWeek",
        "must contain integers 0 through 6",
      );
    }
    if (!nonEmpty(frequency.localTime))
      add(
        issues,
        "INVALID_RECURRENCE_LOCAL_TIME",
        path + ".frequency.localTime",
    ...
    if (
      !Number.isInteger(frequency.dayOfMonth) ||
...
function validateConstraints(
  ...
  if (!record(value)) {
    add(
      issues,
      "INVALID_TIMING_CONSTRAINTS",
      path,
      "constraints must be an object",
    );
    return;
  }
  if (value.earliestStart !== undefined)
    validateAbsoluteTime(value.earliestStart, path + ".earliestStart", issues);
  if (value.latestStart !== undefined)
    validateAbsoluteTime(value.latestStart, path + ".latestStart", issues);
  if (value.minimumDuration !== undefined)
    validateDuration(value.minimumDuration, path + ".minimumDuration", issues);
  if (value.maximumDuration !== undefined)
    validateDuration(value.maximumDuration, path + ".maximumDuration", issues);
}
...
function validateDependency(
  ...
  if (!record(value) || !nonEmpty(value.kind)) {
    add(
      issues,
      "INVALID_TIMING_DEPENDENCY",
      path,
      "dependency kind is required",
    );
    return;
  }
  if (
...
function elapsedMilliseconds(
  duration: TimingDuration | undefined,
): number | undefined {
...
export function validateActionTiming(input: unknown): TimingValidationResult {
  ...
  if (!record(input)) {
    return {
      ...
      issues: [
        {
          code: "INVALID_ACTION_TIMING",
          path: "$",
          message: "ActionTiming must be an object",
        },
      ],
    };
  }
  ...
  strictFields(input, "$", issues);

  if (input.schemaVersion !== ACTION_TIMING_SCHEMA_VERSION) {
    add(
      issues,
      "UNSUPPORTED_TIMING_SCHEMA_VERSION",
      "schemaVersion",
      "supported timing schema is " + ACTION_TIMING_SCHEMA_VERSION,
    );
  }
  ...
  validateTimingValue(
    ...
    "requestedStart",
    issues,
    (value, path) => validateRequestedStart(value, path, issues),
  );
  validateTimingValue(
    input.implementationDelay,
    "implementationDelay",
    issues,
    (value, path) => validateOffset(value, path, issues),
  );
  validateTimingValue(
    input.effectiveStart,
    "effectiveStart",
    issues,
    (value, path) => validateEffectiveStart(value, path, issues),
  );
  validateTimingValue(input.duration, "duration", issues, (value, path) =>
    validateDuration(value, path, issues),
  );
  validateTimingValue(input.end, "end", issues, (value, path) =>
    validateEnd(value, path, issues),
  );
  validateTimingValue(
    input.terminationCondition,
    "terminationCondition",
    issues,
    (value, path) => validateTermination(value, path, issues),
  );
  validateTimingValue(input.recurrence, "recurrence", issues, (value, path) =>
    validateRecurrence(value, path, issues),
  );
  ...
  validateTimingValue(input.constraints, "constraints", issues, (value, path) =>
    validateConstraints(value, path, issues),
  );

  if (!Array.isArray(input.dependencies)) {
    add(
      issues,
      "INVALID_TIMING_DEPENDENCIES",
      "dependencies",
      "dependencies must be an array",
    );
  } else {
    input.dependencies.forEach((entry: unknown, index: number) =>
      validateDependency(entry, "dependencies[" + index + "]", issues),
    );
  }
  ...
  if (
    ...
    add(
      issues,
      "PERSISTENT_ACTION_HAS_END",
      "end",
      "persistent timing cannot also declare a fixed/derived end",
    );
  }

  const duration =
    record(input.duration) && input.duration.state === "SPECIFIED"
      ? (input.duration.value as TimingDuration)
      : undefined;
  const end =
    record(input.end) &&
    input.end.state === "SPECIFIED" &&
    record(input.end.value)
      ? input.end.value
      : undefined;
  const effective =
    record(input.effectiveStart) &&
    input.effectiveStart.state === "SPECIFIED" &&
    record(input.effectiveStart.value)
  ...
  if (
    ...
    if (
      ...
      add(
        ...
        "end",
        "explicit end must agree with elapsed duration anchored at effective start",
      );
    }
  }
...
export function validateTimingDependencyGraph(
  ...
  for (const node of nodes) {
    if (adjacency.has(node.actionId))
      add(
        issues,
        "DUPLICATE_ACTION_ID",
        "actionId",
        "Action identities must be unique",
      );
    adjacency.set(node.actionId, [
      ...node.dependencies.map((dependency) => dependency.actionId),
      ...embeddedReferences(node.timing),
    ]);
  }
...
function embeddedReferences(value: unknown): string[] {
  if (Array.isArray(value)) return value.flatMap(embeddedReferences);
  if (!record(value)) return [];
  return [
    ...(typeof value.actionId === "string" ? [value.actionId] : []),
    ...Object.values(value).flatMap(embeddedReferences),
  ];
}
export function assertValidActionTiming(
  input: unknown,
): asserts input is ActionTiming {
  const result = validateActionTiming(input);
  if (!result.ok)
    throw new Error(result.issues.map((i) => i.code + ":" + i.path).join(", "));
}
function strictFields(
  ...
  if (Array.isArray(value)) {
    value.forEach((v, i) => strictFields(v, `${path}[${i}]`, issues));
    return;
  }
  if (!record(value)) return;
  let allowed: string[];
  const kinds: Record<string, string[]> = {
    ...
    UTC: ["at", "timeZone"],
    LOCAL: ["at"],
    EVENT_RELATIVE: ["relation", "eventId", "offset"],
    TRIGGER_RELATIVE: ["triggerId"],
    ACTION_RELATIVE: ["relation", "actionId", "offset"],
    DERIVE_FROM_REQUESTED_START: [],
    DERIVE_FROM_DURATION: [],
    ELAPSED: ["amount", "unit", "anchor"],
    CALENDAR: ["amount", "unit", "anchor"],
    INSTANTANEOUS: [],
    PERSISTENT: [],
    STATE: ["condition"],
    COMPOSITE: ["operator", "conditions"],
    METRIC_THRESHOLD: ["metricId", "operator", "value", "unit"],
    EVENT_OCCURS: ["eventId"],
    ACTION_STARTS: ["actionId"],
    ACTION_COMPLETES: ["actionId"],
    DAILY: ["interval", "localTime"],
    WEEKLY: ["interval", "daysOfWeek", "localTime"],
    MONTHLY: ["interval", "dayOfMonth", "localTime"],
    CUSTOM_INTERVAL: ["every"],
    BOUNDED: ["recurrenceStart", "recurrenceEnd", "maxOccurrences"],
    OPEN_ENDED: ["explicitlyOpenEnded"],
    START_AFTER: ["actionId", "offset"],
    EFFECTIVE_AFTER: ["actionId", "offset"],
    COMPLETE_AFTER: ["actionId", "offset"],
    END_WITH: ["actionId", "offset"],
    START_AFTER_ACTION_EFFECTIVE: ["actionId", "offset"],
    START_AFTER_ACTION_COMPLETED: ["actionId", "offset"],
    END_WHEN_ACTION_STARTS: ["actionId"],
  };
  if (path === "$")
    allowed = [
      "schemaVersion",
    ...
    allowed =
      value.state === "SPECIFIED"
        ? ["state", "value", "sourceRef"]
        : ["state", "reason"];
  else if ("kind" in value) allowed = ["kind", ...(kinds[value.kind] ?? [])];
  else if (path.endsWith(".at")) allowed = ["localDateTime", "timeZone"];
  else if (path === "recurrence.value") allowed = ["frequency", "boundary"];
  else if (path === "constraints.value")
    allowed = [
      "earliestStart",
      "latestStart",
      "minimumDuration",
  ...
  if (
    (value.kind === "ELAPSED" || value.kind === "CALENDAR") &&
    ![
      "duration.value",
      "constraints.value.minimumDuration",
      "constraints.value.maximumDuration",
    ].includes(path)
  )
    allowed = allowed.filter((key) => key !== "anchor");
  for (const [key, child] of Object.entries(value)) {
    ...
    strictFields(child, path === "$" ? key : path + "." + key, issues);
  }
  try {
    if (value.localDateTime !== undefined)
      Temporal.PlainDateTime.from(value.localDateTime, { overflow: "reject" });
    if (value.localTime !== undefined)
      Temporal.PlainTime.from(value.localTime, { overflow: "reject" });
    if (value.kind === "UTC") Temporal.Instant.from(value.at);
  } catch {
    add(
      issues,
