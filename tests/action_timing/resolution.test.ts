...
describe("Step 12 deterministic timing resolution", () => {
  it("resolves IMMEDIATE against the approved clock without changing canonical intent", () => {
    const resolution = resolveActionTiming(immediatePersistentBudgetTiming, {
      approvedClock,
    });
  ...
  it("resolves an elapsed seven-day window from effective start", () => {
    const resolution = resolveActionTiming(fridaySevenDayBudgetTiming, {
      approvedClock,
    });
    expect(resolution.status).toBe("VALID");
  ...
  it("keeps implementation delay separate from requested start", () => {
    const resolution = resolveActionTiming(delayedBudgetTiming, {
      approvedClock,
    });
    expect(resolution.status).toBe("VALID");
  ...
  it("keeps future event-relative timing unresolved instead of inventing a date", () => {
    ...
    expect(resolution.unresolvedDependencies).toContain(
      "EVENT_NOT_YET_OBSERVED:ORDER_DELIVERED",
    );
  });

  it("resolves calendar offsets in the declared business timezone", () => {
    const resolution = resolveActionTiming(postPurchaseFourteenDaysTiming, {
      approvedClock,
      eventTimes: {
    ...
    expect(resolution.status).toBe("VALID");
    expect(resolution.resolvedEffectiveStart).toBe("2026-10-04T14:00:00.000Z");
  });
